// src/app/teacher/login/page.js
'use client'; // هذا المكون سيكون Client Component

import { useState } from 'react';
import Link from 'next/link';
import { signInTeacher } from '@/lib/actions/auth'; // استيراد Server Action لتسجيل الدخول
...
export default function TeacherLoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await signInTeacher(formData); // استدعاء Server Action
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <div className="bg-white p-8 rounded-xl shadow-lg border border-gray-200 w-full max-w-md">
        <h1 className="text-3xl font-bold mb-6 text-gray-800 text-center">دخول المعلمين</h1>
        ...
        <p className="text-center text-gray-600 text-sm mt-6">
          ليس لديك حساب؟{' '}
          <Link href="/teacher/register" className="text-blue-600 hover:underline font-semibold">
            سجل الآن
          </Link>
        </p>
      </div>
