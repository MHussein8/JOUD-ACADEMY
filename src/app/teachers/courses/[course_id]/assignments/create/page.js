...
'use client'; // يجب أن يكون هذا Component من نوع Client Component

import { useState, useEffect, use } from 'react';
import Link from 'next/link';
import { useRouter } from 'next/navigation'; // لاستخدام الـ router للتحويل بعد الحفظ
import { createAssignmentAction } from '@/lib/actions/assignment'; // استيراد Server Action
import { supabase } from '@/lib/supabaseClient'; // استيراد عميل Supabase الخاص بالمتصفح

export default function CreateAssignmentPage({ params }) {
  const router = useRouter(); // تهيئة الـ router
  
  // استخدام React.use() لفك الـ Promise بتاع params، هذا يحل خطأ الوصول المباشر
  const unwrappedParams = use(params);
  const { course_id } = unwrappedParams; // استخدم الـ params بعد ما اتفكت

  // حالة النموذج (Form State) لتخزين بيانات الواجب
  const [assignmentTitle, setAssignmentTitle] = useState('');
  const [assignmentDescription, setAssignmentDescription] = useState('');
  const [dueDate, setDueDate] = useState('');
  const [assignmentType, setAssignmentType] = useState('homework'); // القيمة الافتراضية 'homework'
  ...
  // حالة الأسئلة
  const [questions, setQuestions] = useState([]); // مصفوفة لتخزين بيانات الأسئلة
  ...
  // استخدام useEffect لجلب الجلسات عند تحميل الصفحة
  useEffect(() => {
    async function fetchSessions() {
      // تحقق إضافي للتأكد من وجود course_id قبل محاولة الجلب
      if (!course_id) {
      ...
      // جلب الجلسات باستخدام عميل Supabase الخاص بالمتصفح
    ...
    setQuestions(
      questions.map((q) => {
        if (q.id === id) {
          ...
          // إعادة تهيئة الحقول بناءً على نوع السؤال الجديد
  ...
  const handleSubmit = async (e) => {
    ...
    // تحويل مصفوفة الأسئلة إلى سلسلة نصية JSON لإرسالها
    formData.append('questionsData', JSON.stringify(questions));

    // استدعاء Server Action لحفظ البيانات
    const result = await createAssignmentAction(formData);

    if (result.success) {
      setSuccessMessage(result.message);
      // إعادة تعيين حالة النموذج بعد الحفظ
      setAssignmentTitle('');
      ...
      setQuestions([]);
      // توجيه المستخدم إلى صفحة الكورس بعد الحفظ
  ...
  // عرض حالة التحميل أثناء جلب الجلسات
  ...
  // إذا كان هناك خطأ في جلب الجلسات، يتم عرض رسالة الخطأ
  if (error) {
    return (
