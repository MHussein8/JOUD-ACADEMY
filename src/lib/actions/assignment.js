...
export async function createAssignmentAction(formData) {
  ...
  const maxScore = parseInt(formData.get('maxScore') || 0);
  ...
  try {
    ...
    for (const q of questionsData) {
      ...
      // تجهيز accepted_variations
      let accepted_variations = null;
      if (q.questionType === 'multiple_choice') {
        accepted_variations = q.options;
      } else if (q.questionType === 'short_text') {
        accepted_variations = q.correctAnswer
    ...
    // تحديث max_score للواجب بعد إضافة كل الأسئلة (لو أردت تحديثه بناءً على مجموع الأسئلة)
...
export async function updateAssignmentAction(formData) {
  ...
  try {
    ...
    const questionsToKeepIds = questionsData.filter(q => q.id).map(q => q.id);
    ...
    // إعادة التحقق من مسار الواجبات لضمان تحديث البيانات في الواجهة
    revalidatePath('/teachers/assignments');
    revalidatePath(`/teachers/courses/${formData.get('courseId')}`);
